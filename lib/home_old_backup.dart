...
class _HomeBackupState extends State<HomeBackup> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _textStreamSubscription = ReceiveSharingIntent.getTextStream().listen((String textStream) async {
      print("[Home] getTextStream() - textStream : $textStream");

      if (textStream.isNotEmpty) {
        clearData();
        await updateLinkInfo(textStream);
      }
    }, onError: (err) {
    ...
    ReceiveSharingIntent.getInitialText().then((String? initialText) async {
      print("[Home] getInitialText() - initialText : $initialText");
      if (initialText != null && initialText.isNotEmpty) {
        clearData();
        await updateLinkInfo(initialText);
      }
    }, onError: (err) {
      print("[Home] getInitialText() - error: $err");
    });

    // _textUriStreamSubscription = ReceiveSharingIntent.getTextStreamAsUri().listen((Uri uri) {
    //   print("[Home] getTextStreamAsUri() - uri: $uri");
    // }, onError: (err) {
    //   print("[Home] getTextStreamAsUri() - error: $err");
    // });

    // ReceiveSharingIntent.getInitialTextAsUri().then((Uri? uri) {
    //   print("[Home] getInitialTextAsUri() - uri: $uri");
    // }, onError: (err) {
    //   print("[Home] getInitialTextAsUri() - error: $err");
    // });
  }
  ...
  Future<void> updateLinkInfo(String? textUrl) async {
    print('[Home] updateLinkInfo() - textUrl = $textUrl');
    ...
    if (textUrl == null || textUrl.isEmpty) {
      ...
      if (!textUrl.contains('http')) {
        setState(() {
          _sharedDescription = 'Shared Data is not link';
        });
        return;
      }

      var url = '';
      var values = textUrl.split(RegExp(r'\s'));
      url = values.last;
      ...
      try {
        ...
        if (data.body.isEmpty) {
          setState(() {
            _sharedTitle = textUrl;
          });
        } else {
          BeautifulSoup soup = BeautifulSoup(utf8.decode(data.bodyBytes));
